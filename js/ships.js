...
function Ship(posx, posy, posz, imageSRC, imageWidth, imageHeight) {
	...
	this.update = function() {
		...
		this.posX += this.velX - 2.25;
		this.posY += this.velY - 2.25; 
		this.posZ += this.velZ - 2.25; 
	...
	this.render = function() {
		var dom = this.domElement,
			styleStr,
			sx = Math.atan(counter*0.4)*0.04 + this.size, 
			sy = Math.atan(Math.PI + counter*0.4)*0.04 + this.size; 
